...
let sourceUrlColumnPromise = null;

async function ensureSourceUrlColumn() {
    if (!sourceUrlColumnPromise) {
        sourceUrlColumnPromise = pool.query(
            `ALTER TABLE review_rewards ADD COLUMN IF NOT EXISTS source_url TEXT`
        ).catch((err) => {
            sourceUrlColumnPromise = null;
            throw err;
        });
    }
    return sourceUrlColumnPromise;
}

function normalizeSourceUrl(value, req) {
    const raw = sanitizeInput(value);
    if (!raw || typeof raw !== 'string' || raw.length > MAX_SOURCE_URL_LENGTH) return null;
    ...
    if (!requestHost || sourceHost !== requestHost) return null;
...
async function handleGetSubmissions(req, res) {
    try {
        ...
        let query = `SELECT id, payment_method, payment_handle, review_link, status,
                     ...
                     reservation_id, verification_method, verification_status, verified_reservation_id,
                     verified_previously_used, provided_checkin, provided_checkout,
                     actual_checkin, actual_checkout, source_url,
...
async function handleCreateSubmission(req, res) {
    try {
        ...
        const sourceUrl = normalizeSourceUrl(body.source_url, req);
